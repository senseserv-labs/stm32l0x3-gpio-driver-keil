...
void hal_gpio_configure_pin_mode(GPIO_TypeDef *GPIOx, uint16_t pin_no, uint32_t mode)
{
	...
	GPIOx->MODER   &= ~(0x03 << ( 2 /* 2 bits */ * pin_no));
	GPIOx->MODER   |= (mode << ( 2 /* 2 bits */ * pin_no));
}
...
{
	 GPIOx->OSPEEDR |= (speed << (2 /* 2 bits */ * pin_no));
}
...
{
	 GPIOx->PUPDR |= (pupd << (2 /* 2 bits */ * pin_no));
}
...
void hal_gpio_set_alt_function(GPIO_TypeDef *GPIOx, uint16_t pin_no, uint16_t alt_fun_value)
{
  if ( pin_no  <= 7 )
	{
	  	/* Low Register */
		GPIOx->AFR[0] |= (alt_fun_value << ((pin_no) * 4 /* 4 bits */ ));
	}
	else
	{
		/* High Register */
		GPIOx->AFR[1] |= (alt_fun_value << (( pin_no % 8) * 4 /* 4 bits */));
	}
}
